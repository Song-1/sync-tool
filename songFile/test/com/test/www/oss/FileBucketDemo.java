...
public class FileBucketDemo {
	...
	// 运行前先设置好以下三个参数
	private static final String BUCKET_NAME = "musicone";
	private static final String OPERATOR_NAME = "mxb";
	private static final String OPERATOR_PWD = "wobuzhidao";
	...
	public static void main(String[] args) throws Exception {
		...
		UpYun.init();
		// 初始化空间
		upyun = new UpYun(UpYun.BUCKET_NAME, UpYun.OPERATOR_NAME, UpYun.OPERATOR_PWD);
...
//		testRmDir();
	...
	public static void testWriteFile() throws IOException {
		 ...
		 //目录
        File dataDir = new File("E:/1");
        //存放目录及其子目录下的所有文件对象
        List<File> myfile = new ArrayList<File>();
        //开始遍历
        GetAllFileByPath.listDirectory(dataDir, myfile);
        
        System.out.println("目录下包含 " + myfile.size() + "个文件：");
        for(File file : myfile){
        	long startTime =  System.currentTimeMillis();
    		boolean result3 = upyun.writeFile("/upyun/"+file.getName() + file.getAbsolutePath().substring(file.getAbsolutePath().lastIndexOf("."), file.getAbsolutePath().length()), file, true);
    		System.out.println("上传 " + file.getAbsolutePath() + isSuccess(result3));
    		long endTime = System.currentTimeMillis();
    		if ((endTime-startTime) !=0 && (endTime-startTime)/1000 != 0) {
    			System.out.println(file.length()/((endTime-startTime)/1000));
    		}
        }
